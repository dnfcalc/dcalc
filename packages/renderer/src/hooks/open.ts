...
export function useOpenWindow(opt?: UseOpenOption & { immediate?: boolean }) {
  ...
  return async (maybeUrl: MaybeRef<string> | (() => string), { width = 0, height = 0, target = "_self", query = {}, title = "DNF计算器 & Colg", scale = 1.0 }: UseOpenOption = {}) => {
    ...
    try {
      ...
      if (_target == "_blank" || width * height < 1) {
        if (window.pywebview) {
          window.pywebview.api.invoke("open-url", url)
        } else {
          window.open(url, _target as string)
        }
      } else if (_target == "_self") {
        if (window.pywebview) {
          window.pywebview.api.invoke("open-win", {
            url,
            width,
            height
          })
