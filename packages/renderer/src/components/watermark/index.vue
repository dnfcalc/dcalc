...
import { computed, onBeforeUnmount, onMounted, onUnmounted, shallowRef, unref, watch } from "vue"
...
const showContent = computed(() => !appstore.tryChange ? props.content : "营销号死全家")

const showColor = computed(() => !appstore.tryChange ? props.color : "red")
...
const appendWatermark = (base64Url: string, markWidth: number) => {
  if (containerRef.value && watermarkRef.value) {
    ...
    if (props.fullscreen) {
      htmlRef.value.setAttribute("style", "position: absolute")
