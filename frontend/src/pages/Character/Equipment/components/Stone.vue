  ...
  <div>
    ...
    <template v-for="(equip,_) in suitEquipments" :key="_">
      <template v-if="equip?.rarity == curRarity || !curRarity">
        <EquipmentIcon :equipment="equip" :inactive="configStore.config.equips[equip?.itemDetailType ?? '']?.fusion != equip?.id" @click="configStore.chooseEqu(equip,true)"/>
      </template>
...
const suitEquipments = computed(() => {
  const equs =  infoStore.stones.filter((a) => a.suit.includes(curSuit.value.toString()) && a.suit.length === 1 )

  const res: (IEquipment | undefined)[] = []
  const raritiyList = ['神器','传说','史诗']

  raritiyList.forEach((rarity) => {
    const equ : (IEquipment | undefined)[]  = equs.filter((a) => a.rarity === rarity)
