...
<template>
  <div class="flex flex-wrap gap-4px content-start" :style="{ width: `${rowCount * 32}px` }">
    <template v-for="(equ, _) in equs" :key="_">
      <EquipmentIcon :equipment="equ" :inactive="configStore.config.equips[equ?.itemDetailType ?? '']?.id != equ?.id" @click="configStore.chooseEqu(equ)"/>
    </template>
  </div>
</template>
...
const equs = computed(() => {
  const items = infoStore.equips.filter(
    (equip) => ['称号','宠物'].includes(equip.itemDetailType),
  )
  ...
  const treasure = infoStore.equips.filter(
    (equip) => equip.categorize.includes('秘宝'),
  )
