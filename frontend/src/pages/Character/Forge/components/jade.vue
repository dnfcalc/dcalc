  ...
  <template v-for="key in ['0','1','2','3']" :key="key">
    <div class="flex gap-2 mt-5px">
      <calc-select class="flex-1 !h-20px" v-model="configStore.config.jades[key].id">
        <calc-option v-for="(item, index) in ['0','1']" :key="index" :value="item">
          {{item}}
        </calc-option>
      </calc-select>
      <calc-select class="flex-1 !h-20px" v-model="configStore.config.jades[key].value"></calc-select>
    </div>
  </template>
</div>
</template>
...
//         } else {
//           const res = []
//           const cur = infoStore.jade?.find(e => e.id == id) as IJadeInfo
//           if (cur) {
