...
async def get_skills_list(
    ...
    data = (
        list(
            filter(
                lambda x: re.sub(r'\s+', '', skillName) in re.sub(r'\s+', '', x['skillName']),
                skills_list,
            )
        )
        if skillName
...
async def get_jobs_info(
    ...
    for adv in adventure_info:
        matched_children = [child for child in adv.get('children', []) if jobName in child.get('title', '') or jobName in child.get('name', '')]
        if matched_children:
            ...
            if 'levelInfo' in skill_info and 'rows' in skill_info['levelInfo']:
                ...
                if detail and len(detail) > 0:
                    ...
                    for key in skill_info['levelInfo']['optionValue'].keys():
                        skill_info['levelInfo']['detail'] = skill_info['levelInfo']['detail'].replace(
                            f'{{{key}}}',
