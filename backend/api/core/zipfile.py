...
class ZipInfo(object):
    ...
    def FileHeader(self, zip64=None):
        ...
        header = struct.pack(
            structFileHeader, stringFileHeader, self.extract_version, self.reserved, flag_bits, self.compress_type, dostime, dosdate, CRC, compress_size, file_size, len(filename), len(extra)
        )
                ...
                for filename in dirlist:
                    ...
                    if os.path.isdir(path):
                        if os.path.isfile(os.path.join(path, '__init__.py')):
                            # This is a package directory, add it
                            self.writepy(path, basename, filterfunc=filterfunc)  # Recursive call
                    elif ext == '.py':
