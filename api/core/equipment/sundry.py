...
def sundries_27063(char: CharacterProperty = {}, mode=0, text=False, *args):
    if text:
        return "(235,'New','四维(48)|三攻(30)')"
    if mode == 0:
        # 勋章基础
        char.基础属性加成(四维=48, 三攻=30)
        # 勋章品质
        # 攻击强化 BUFF量 名望
        medal_quality = [(1035, 125, 235), (2045, 249, 315), (3799, 461, 415),
                         (5960, 724, 509), (8532, 1035, 606), (11688, 1418, 708)]
        quality = medal_quality[args[0]]
        char.攻击强化加成(quality[0]*args[3])
        char.buff量加成(quality[1])
        # 勋章强化
        medal_lv = [(0, 0, 0), (390, 47, 24), (779, 95, 47), (1169, 142, 71), (1558, 189, 94), (1948, 236, 118), (2337, 284, 142),
                    (2727, 331, 165), (3117, 378, 189), (3506, 425, 212), (3896, 473, 236), (4285, 520, 259), (4675, 567, 283)]
        lv = medal_lv[args[1]]
        char.攻击强化加成(lv[0]*args[3])
        char.buff量加成(lv[1])
        # 宝石
        char.攻击速度增加(0.03)
        char.所有属性强化加成(7)
        medal_gems_quality = [(224, 27, 46), (403, 49, 58), (701, 85, 72),
                              (1140, 138, 88), (1753, 212, 106)]
        for i in args[2]:
            gems_quality = medal_gems_quality[i]
            char.攻击强化加成(gems_quality[0]*args[3])
            char.buff量加成(gems_quality[1])
        pass
    if mode == 1:
        pass
...
# 杂项id范围 27001~27999
for i in range(27001, 27100):
    try:
        if i not in sundries_list.keys():
            sundries_list[i] = eval('sundries_{}'.format(i))
    except Exception:
        pass


def get_sundriesfunc_by_id(id):
    return sundries_list.get(id, sundries_27000)


def get_sundries_setinfo():
    infolist = []
    for i in sundries_list.keys():
        data = {}
        data['id'] = i
        info = eval(sundries_list[i](text=True))
        num = 0
        for k in index:
            data[k] = info[num]
            num += 1
